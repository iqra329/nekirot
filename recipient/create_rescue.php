...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title || !$description || !$contact_phone || !$latitude || !$longitude) {
        ...
        $stmt = $db->prepare('INSERT INTO rescues (recipient_id, title, description, contact_phone, latitude, longitude, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())');
        $status = 'pending';
        ...
        $error = 'Unable to create rescue request. Please try again.';
    }
}
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card shadow-sm">
            <div class="card-body">
                <h2 class="card-title mb-4">Request Rescue</h2>
                <?php if ($error): ?>
                    <div class="alert alert-danger"><?= htmlspecialchars($error) ?></div>
                <?php endif; ?>
                <form method="post" action="<?= BASE_URL ?>recipient/create_rescue.php">
                    <div class="mb-3">
